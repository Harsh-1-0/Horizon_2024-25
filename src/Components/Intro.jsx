...
function Intro({ onFinish, onPlayAudio }) {
  ...
  return (
    <div className="bg-[#FEFBEC] min-h-screen h-screen">
      <CSSTransition in={showText} timeout={500} classNames="fade" unmountOnExit appear>
      ...
      <CSSTransition in={showOptions} timeout={500} classNames="fade" unmountOnExit>
        <div className="flex flex-col justify-center items-center h-screen">
        <h1 className="text-center text-black font-Gilroy text-5xl lg:text-7xl lowercase">
          ...
          <div className="flex flex-row items-center mt-8 space-x-10">
            <button
              className="px-10 py-2 border-2 border-black text-black bg-transparent hover:bg-[#F2C6AB] shadow-custom font-poppins font-semibold"
              onClick={handleOption('yes')}>Yes</button>
            <button
              className="px-10 py-2 border-2 border-black text-black bg-transparent hover:bg-[#F2C6AB] shadow-custom font-poppins font-semibold"
              onClick={handleOption('no')}>No</button>
          </div>
        </div>
      </CSSTransition>
      <CSSTransition in={playGif} timeout={200} classNames="fade-fast" unmountOnExit>
